...
module down_ser8b10(input CLK10, input RESET_OUT_N, 
			   input scramble,
			   output [9:0]DO,	output XMT_READY, output XMT_RD,
			   input k, input [7:0]in, input ready); // low speed

	reg		r_reset;
	always @(posedge CLK10)
		r_reset <= ~RESET_OUT_N;

	reg	   [9:0]c_d;	// 7/10 clock setup to r_d from everywhere except r_d and r_count
	assign DO = c_d;
	reg			r_ready;	// 3/10 clock setup from ready
	assign		XMT_READY = r_ready;
